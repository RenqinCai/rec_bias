...
class MOVIE_LOADER(object):
    ...
    def __iter__(self):
        ...
        user_num = len(self.m_user_itemlist_dict)
        batch_num = user_num // batch_size
        print("batch num", batch_num)

        for batch_index in range(batch_num):
            user_iter, positem_iter, negitem_iter = self._sample()
            ...
            yield user_iter_tensor, positem_iter_tensor, negitem_iter_tensor

    def _sample(self):
        
        user_num = self.m_user_num
        item_num = self.m_item_num

        userid_list = np.arange(1, user_num, 1)
        itemid_list = np.arange(1, item_num, 1)

        userids = np.random.choice(userid_list, size=self.m_batch_size, replace=False)
        ...
        for idx, user in enumerate(userids):
            pos_items = self.m_user_itemlist_dict[user]
            pos_itemnum = len(pos_items)

            if pos_itemnum == 0:
                continue

            pos_item = np.random.choice(pos_items)
            neg_item = np.random.choice(itemid_list)
            while neg_item in pos_items:
                neg_item = np.random.choice(itemid_list)
            
            # batch_data.append([user, pos_item, neg_item])
            batch_user.append(user)
            batch_positem.append(pos_item)
            batch_negitem.append(neg_item)

        return batch_user, batch_positem, batch_negitem
