...
class DATA():
    ...
    def f_load_movie(self, args):
        ...
        train_item_num = train_df.pos_itemid.nunique()
        print("train item num", train_item_num)

        train_data = MOVIE(args, train_df)
        ...
        valid_loader = DataLoader(dataset=valid_data, batch_size=batch_size, shuffle=True, num_workers=8, collate_fn=valid_data.collate)

        test_loader = DataLoader(dataset=test_data, batch_size=batch_size, shuffle=True, num_workers=8, collate_fn=test_data.collate)
