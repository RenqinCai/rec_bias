...
class MOVIE(Dataset):
    def __init__(self, args, df, unique_item_num):
        ...
        # ### counts the frequency of a user and get the group ids [0, 1]
        # user_freq_dict = dict(df["userid"].value_counts())
        # sorted_user_list = [k for k, v in sorted(user_freq_dict.items(), key=lambda x: x[1], reverse=True)]
        
        # self.m_user_group_dict = {}
        # group0_user_num = int(unique_user_num*0.05)
        # group0_user_list = sorted_user_list[:group0_user_num]
        # for user in group0_user_list:
        #     self.m_user_group_dict[user] = 0

        # group1_user_list = sorted_user_list[group0_user_num:]
        # for user in group1_user_list:
        #     self.m_user_group_dict[user] = 1


        ### counts the frequency of a user and get the group ids
        user_freq_dict = dict(df["userid"].value_counts())
        sorted_user_list = [k for k, v in sorted(user_freq_dict.items(), key=lambda x: x[1], reverse=True)]
        ...
        group0_user_list = sorted_user_list[:group0_user_num]
        for user in group0_user_list:
            self.m_user_group_dict[user] = user_freq_dict[user]
    ...
    def __getitem__(self, idx):
        ...
        usergroup_i = self.m_user_group_dict[user_i]
    
        return user_i, itemlist_i, self.m_unique_item_num, usergroup_i
    ...
    def collate(batch):
        ...
        for i in range(batch_size):
            ...
            usergroup_i = sample_i[3]
            usergroup_iter.append(usergroup_i)
        ...
        usergroup_iter_tensor = torch.from_numpy(np.array(usergroup_iter)).long()

        return user_iter_tensor, positem_iter_tensor, negitem_iter_tensor, positemnum_iter_tensor, usergroup_iter_tensor
